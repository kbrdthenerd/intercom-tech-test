import { writeCustomers } from '../customerUtils'
import customers from './testData/customersPretty.json'
import { promises } from 'fs'

describe('Writing customers to files', () => {
  test('Should write correct customer data to a file', async () => {
    await promises.unlink('./output.txt')
    await writeCustomers(customers)
    const outputText =  await promises.readFile('./output.txt', {encoding: 'utf8'})
    expect(outputText).toEqual(expectedOutput)
  })
})

const expectedOutput = `{"user_id":4,"name":"Ian Kehoe"}
